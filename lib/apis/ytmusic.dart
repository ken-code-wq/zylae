...
class YtMusicService {
  ...
  static const userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:88.0) Gecko/20100101 Firefox/88.0';
  ...
  Future<Map> sendRequest(
    ...
    if (response.statusCode == 200) {
      ...
      Logger.root.severe('YtMusic returned ${response.statusCode}', response.body);
  ...
  Future<List<Map>> search(
    ...
    try {
      ...
      final List finalResults = NavClass.nav(results, ['sectionListRenderer', 'contents']) as List? ?? [];
      for (final sectionItem in finalResults) {
        final bool containsHeader = (sectionItem as Map).containsKey('musicCardShelfRenderer');
        final sectionSearchResults = [];
        final String sectionSelfRenderer = containsHeader ? 'musicCardShelfRenderer' : 'musicShelfRenderer';
        ...
        final List sectionChildItems = NavClass.nav(sectionItem, [sectionSelfRenderer, 'contents']) as List? ?? [];
  ...
  Future<List<String>> getSearchSuggestions({
    ...
    try {
      final body = Map.from(context!);
      body['input'] = query;
      final Map response = await sendRequest(endpoints['search_suggestions']!, body, headers);
  ...
  Map? parseInfoFromSubtitle(
    ...
    final String title = NavClass.joinRunTexts(
      NavClass.nav(
        childItem,
        titleNavPath ?? [...NavClass.mRLIRFlex, 0, NavClass.mRLIFCR, ...NavClass.textRuns],
      ) as List?,
    );
    final String subtitle = NavClass.joinRunTexts(
      NavClass.nav(
        childItem,
        subtitleNavPath ?? [...NavClass.mRLIRFlex, 1, NavClass.mRLIFCR, ...NavClass.textRuns],
      ) as List?,
    );
    ...
    final List idNav = (type == 'Song' || type == 'Video') ? NavClass.mrlirPlaylistId : NavClass.mrlirBrowseId;
    final String? id = NavClass.nav(childItem, idNavPath ?? idNav)?.toString() ?? NavClass.nav(childItem, idNavPath2 ?? idNav)?.toString();
  ...
  Future<Map> getSongData({required String videoId}) async {
    ...
    try {
      ...
      final Map response = await sendRequest(endpoints['get_song']!, body, headers);
      int maxBitrate = 0;
      String? url;
      final formats = await NavClass.nav(response, ['streamingData', 'formats']) as List;
      ...
      //   }
      // }
      final videoDetails = await NavClass.nav(response, ['videoDetails']) as Map;
  ...
  Future<Map> getAlbumDetails(String albumId) async {
    ...
    try {
      ...
      final List<Map> songResults = [];
      for (final item in finalResults) {
        final String id = NavClass.nav(item, NavClass.mrlirPlaylistId).toString();
        final String image = NavClass.nav(item, [
          NavClass.mRLIR,
          ...NavClass.thumbnails,
          0,
  ...
  Future<List<String>> getWatchPlaylist({
    ...
    try {
      ...
      for (final item in playlist) {
        if (count > 0) {
          final String id = NavClass.nav(item, ['playlistPanelVideoRenderer', 'videoId']).toString();
          songResults.add(id);
        } else {
          count++;
        }
