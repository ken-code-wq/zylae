...
class AudioPlayerHandlerImpl extends BaseAudioHandler with QueueHandler, SeekHandler implements AudioPlayerHandler {
  ...
  Stream<List<IndexedAudioSource>> get _effectiveSequence =>
      Rx.combineLatest3<List<IndexedAudioSource>?, List<int>?, bool, List<IndexedAudioSource>?>(_player!.sequenceStream, _player!.shuffleIndicesStream, _player!.shuffleModeEnabledStream, (sequence, shuffleIndices, shuffleModeEnabled) {
  ...
  Future<void> _init() async {
    ...
    preferredMobileQuality = Hive.box('settings').get('streamingQuality', defaultValue: '96 kbps').toString();
    preferredWifiQuality = Hive.box('settings').get('streamingWifiQuality', defaultValue: '320 kbps').toString();
    ...
    cacheSong = Hive.box('settings').get('cacheSong', defaultValue: true) as bool;
    ...
    Rx.combineLatest4<int?, List<MediaItem>, bool, List<int>?, MediaItem?>(_player!.currentIndexStream, queue, _player!.shuffleModeEnabledStream, _player!.shuffleIndicesStream, (index, queue, shuffleModeEnabled, shuffleIndices) {
          ...
          if (mediaItem.genre == 'YouTube') {
            ...
            if ((DateTime.now().millisecondsSinceEpoch ~/ 1000) + 350 > expiredAt) {
              ...
              if (Hive.box('ytlinkcache').containsKey(mediaItem.id)) {
                final Map cachedData = Hive.box('ytlinkcache').get(mediaItem.id) as Map;
  ...
  Future<void> startService() async {
    ...
    if (withPipeline && Platform.isAndroid) {
      ...
      _equalizer.parameters.then((value) {
        ...
        bands.map(
          (e) {
            final gain = Hive.box('settings').get('equalizerBand${e.index}', defaultValue: 0.5) as double;
  ...
  void _playbackError(err) {
    ...
    if (err is PlatformException && err.code == 'abort' && err.message == 'Connection aborted') return;
    _onError(err, null);
  }
