...
import 'package:zylae/Helpers/matcher.dart';
import 'package:zylae/Helpers/spotify_helper.dart';
...
class Lyrics {
  static Future<Map<String, String>> getLyrics({
    ...
    print(result['lyrics'] != '' ? "From spotify" : "Not spotify");
    if (result['lyrics'] == '') {
      ...
      if (saavnHas) {
        ...
        result['lyrics'] = await getMusixMatchLyrics(title: title, artist: artist);
        result['type'] = 'text';
        result['source'] = 'Musixmatch';
        if (result['lyrics'] == '') {
          Logger.root.info('Lyrics not found on Musixmatch, searching on Google');
          result['lyrics'] = await getGoogleLyrics(title: title, artist: artist);
          result['type'] = 'text';
      ...
      function: (String accessToken) async {
        ...
        try {
          ...
          try {
            ...
            artist2 = value['tracks']['items'][0]['artists'][0]['name'].toString();
          } catch (e) {
            Logger.root.severe(
          ...
          print('Error in getSpotifyLyrics $e');
        }
      },
      forceSign: false,
    );
  ...
  static Future<String> getSpotifyID(
    String title,
    String artist,
  ) async {
    String result = '';
      ...
      function: (String accessToken) async {
        ...
        try {
          ...
          try {
            ...
            print(
              'Error in extracting artist/title in getSpotifyLyrics for $title - $artist $e',
            );
          }
          final trackId = value['tracks']['items'][0]['id'].toString();
          result = trackId;
      ...
      if (res.statusCode == 200) {
        ...
        if (lyricsData['error'] == false) {
          ...
          if (lyricsData['syncType'] == 'LINE_SYNCED') {
            result['lyrics'] = lines.map((e) => '[${e["timeTag"]}]${e["words"]}').toList().join('\n');
            result['type'] = 'lrc';
          } else {
        ...
        print('getSpotifyLyricsFromId returned ${res.statusCode} ${res.body}');
      }
      return result;
    } catch (e) {
      print('Error in getSpotifyLyrics $e');
