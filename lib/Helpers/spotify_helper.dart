...
Future<String?> retriveAccessToken() async {
  Hive.box('settings').put('spotifyRefreshToken', 'AQD7ALp6fF45HjvFLtAbgRRT-FJjmukafizqfsyMeip-HMlWD4Fyhjg4hCWsMjtlJia74MxYN23DyLjrmsSvKbc6a9le4YdzYu2PsZDTUclcl21NjMEqzh7F0nXW-nyw9PA');
  String? accessToken = Hive.box('settings').get('spotifyAccessToken', defaultValue: null)?.toString();
  String? refreshToken = Hive.box('settings').get('spotifyRefreshToken', defaultValue: null)?.toString();
  final double expiredAt = Hive.box('settings').get('spotifyTokenExpireAt', defaultValue: 0.0) as double;
  print("AToken $accessToken");
  print("ARToken $refreshToken");
  ...
  if (accessToken == null && refreshToken == null) {
    ...
    if ((currentTime + 60) >= expiredAt) {
      ...
      if (data.isNotEmpty) {
        ...
        Hive.box('settings').put('spotifyTokenExpireAt', currentTime + double.parse(data[2]));
      }
    }
    return accessToken;
  }
